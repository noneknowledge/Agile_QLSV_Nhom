...
namespace lab7___ADO.NET
{
    public partial class FormHome : Form
    {
        ...
        private void FormHome_Load(object sender, EventArgs e)
        {
            ...
            var dsLophp = db.LOPHPs.Select(p => new { p.MALHP, p.MAHP,p.MALOP });
            var dsHp = db.HOCPHANs.Select(p => new { p.MAHP,p.TENHP, p.SOTC,p.MABM});
            var dsDiem = db.DKHPs;
            var dsDiemHp = dsLophp.Join(dsHp, a => a.MAHP, b => b.MAHP, (a, b) => new { a.MAHP, b.TENHP, b.SOTC, a.MALHP })
                .Join(dsDiem, x => x.MALHP, y => y.MALHP, (x, y) => new { x.TENHP, y.DIEMTBHE10,y.MASV,x.SOTC ,x.MALHP});
            var dsDiemCaNhan = dsDiemHp.Where(sv => sv.MASV == Masv);       
            foreach (var diem in dsDiemCaNhan)
            {
                chartDiem.Series["Diem"].Points.AddXY(diem.TENHP, diem.DIEMTBHE10);               
            }
